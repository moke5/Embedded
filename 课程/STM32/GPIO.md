...
  GPIO**（General Purpose Input/Output，通用输入 / 输出口**）,即通用I/O(输入/输出)端口，是STM32可控制的引脚。STM32芯片的GPIO引脚与外部设备连接起来，可实现与外部通讯、控制外部硬件或者采集外部硬件数据的功能。
...
- 浮空输入（即不连接内部上下拉电阻）（Floating Input）

> 复位后 ps:CRL\CRH32位
> GPIOX_CRL(CRH)->CNF = 01	浮空输入
>
> GPIOX_CRL(CRH)->MODE = 00	输入模式
...
这些位为**只读**并只能以字(16位)的形式读出。读出的值为对应I/O口的状态。

#### 4种输出模式

- 开漏输出（带上拉或者下拉）（Open-Drain Output）
- 复用开漏输出（带上拉或者下拉）（Alternate Function Open-Drain）
- 推挽输出（带上拉或者下拉）（Push-Pull Output）
- 复用推挽输出（带上拉或者下拉）（Alternate Function Push-Pull）
...
这些位只能写入并只能以字(16位)的形式操作。
 0：对对应的ODRy位不产生影响 1：清除对应的ODRy位为0 注：如果同时设置了BSy和BRy的对应位，BSy位起作用。
...
**在STM32中，一个物理引脚除了作为通用的输入/输出（GPIO）之外，还可以被配置为内部外设（如USART、SPI、I2C等）的连接引脚，这被称为复用功能。**
...
**5. 上拉输入（Input Pull-Up）**

![img](./img/56832.png)

**特点：**

- 默认高电平：引脚悬空时：自动被拉到高电平

- 稳定状态：确保引脚有确定的逻辑电平

 

**6. 下拉输入（Input Pull-Down）**

![img](./img/56833.png)

**特点：**

- 默认低电平：引脚悬空时：自动被拉到低电平
