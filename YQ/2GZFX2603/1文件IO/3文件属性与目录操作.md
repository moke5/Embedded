...
![image-20260713182552366](./img/image-20260713182552366.png)
...
// 目录项结构体
struct dirent {
               ino_t          d_ino;       /* Inode number 文件名所对应的索引号*/
...
![image-20260713182915076](./img/image-20260713182915076.png)

- 文件判断和目录检索

```C
// 拼接路径
static char *jion_path(char const *path, char const *name) {
        int len = strlen(path) + strlen(name) + 2; // '/' + '\0'
        char *ret = (char *)malloc(len);
        if (ret == NULL) {
                return NULL;
        }
        snprintf(ret, len, "%s/%s", path, name);
        return ret;
}

// 入口函数：判断文件类型，分发处理
int bmp_scan(Picture *head, char const *path)
{
        if (&head->list == NULL || path == NULL) {
                return -1;
        }
        ...
        if (S_ISREG(st.st_mode)) {
                ...
                if(ext != NULL && strcmp(ext, ".bmp") == 0) {
                        pic_list_add_tail(head, (char *)path);
                }
                return 0;
        }

        if (S_ISDIR(st.st_mode)) {
                return bmp_search(head, path);
        }
        return -1;
}

// 目录递归搜索
int bmp_search(Picture *head, char const *path) {
        if (&head->list == NULL || path == NULL) {
                return -1;
        }
        ...
        while ((dp = readdir(dir)) != NULL) {
                ...
                if (dp->d_type == DT_REG) {
                        char *str = strrchr(dp->d_name, '.');
                        if (str != NULL && strcmp(str, ".bmp") == 0) {
                                char *full_path = jion_path(path, dp->d_name);
                                if (full_path != NULL) {
                                        pic_list_add_tail(head, full_path);
                                        free(full_path);
                                }
                        }
                } else if (dp->d_type == DT_DIR) {
                        char *subpath = jion_path(path, dp->d_name);
                        if (subpath != NULL) {
                                bmp_search(head, subpath);
                                free(subpath);
                        }
                }
        }
...
int main() 
{  
    List_t *bmpList = dir_searchToList("./aa",".bmp");
    List_t *txtList = dir_searchToList("./aa",".txt");
...
![image-20260713182954973](./img/image-20260713182954973.png)
