...
![image-20260713181616226](./img/image-20260713181616226.png)
...
```C
/* Standard file descriptors. 系统IO */
...
```C
/* Standard streams. 标准IO */
struct _IO_FILE *stdin;		    -> 标准输入的文件指针
struct _IO_FILE *stdout;	    -> 标准输出的文件指针
struct _IO_FILE *stderr;	    -> 标准出错的文件指针
...
函数作用：feof()用来侦测是否读取到了文件尾, 尾数 stream 为 fopen()所返回之文件指针.

参数：你要判断哪个文件，将这个文件的文件指针传递进来
...
函数作用：获取文件当前的位置，也就是当前光标位置到文件开头的距离（字节数）

参数：

你要获取哪个文件的位置，将这个文件的文件指针传递进来
...
返回值 如果文件流有错误发生则返回非 0 值. 

 

**5.3重新定位文件流指针  -> fseek()  -> man 3 fseek**
...
void rewind(FILE *stream);   //fseek(file,  0, SEEK_SET)
