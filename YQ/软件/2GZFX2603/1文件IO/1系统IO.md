...
> 系统IO又称无缓存
>
> 并不是没有使用缓存，调用read和write函数向内核发送请求时，内核会将要读写的数据写入系统内存的缓冲区中，待系统的缓冲区存满时，再对数据统一进行一次操作。
...
![image-20260408180650202](./img/image-20260408180650202.png)
...
文件本质上是一组数据的集合。在逻辑层面上，文件由一系列连续的字节流组成，这一系列字节流以文件结束符（EOF，end of file）作为结束符。

在屋里层面上，每一个存储在磁盘上的文件都由以下两部分组成。

- 数据块：存储文件内容的磁盘块
- inode：用于描述文件属性信息的数据结构，也称为索引节点，其中存储这文件的属性信息，如文件名称、文件大小、文件权限等。在同一个文件系统中，每一个索引节点都有唯一的标识号，称为索引节点编号（inode号）

```
fd				fd是一个整数编号，用于索引fd table。
负责标识	

fd table		fd table用于维护 fd -> ofd 的映射关系。
负责查找

ofd				ofd用于维护一次打开行为的状态。
负责访问状态
...
使用`ls -l`命令可以查看文件的部分属性信息，使用 `ls -i`命令可以查看文件的inode号，而 `stat` 命令几乎可以查看文件的所有属性信息。

```shell
gec@ubuntu:/mnt/hgfs/demo1$ stat bmp1.bmp 
  File: bmp1.bmp
  Size: 16754742        Blocks: 32725      IO Block: 1024   regular file
Device: 36h/54d Inode: 1615        Links: 1
Access: (0777/-rwxrwxrwx)  Uid: (    0/    root)   Gid: (    0/    root)
Access: 2026-04-27 21:40:53.000000000 +0800
Modify: 2026-04-27 21:40:53.000000000 +0800
Change: 2026-04-27 21:40:53.000000000 +0800
 Birth: -
```
...
--------------------------------------------------
  File: bmp1.bmp             ← 文件名
  Size: 16754742             ← 文件大小：约16MB
  Blocks: 32725              ← 占用的块数（传统一块512B，实际占用磁盘空间 = 32725×512 ≈ 16.7MB）
  IO Block: 1024             ← 文件系统一次IO读写的块大小
  regular file               ← 文件类型：普通文件（7种之一）
Device: 36h/54d              ← 所在设备的主/次设备号（这里是vmware共享文件夹）
Inode: 1615                  ← 文件的inode号（文件系统内唯一编号）
Links: 1                     ← 硬链接计数（目前只有自身）
Access: (0777/-rwxrwxrwx)    ← 权限：所有人可读可写可执行
Uid: (    0/    root)        ← 文件属主：root
Gid: (    0/    root)        ← 文件属组：root
Access: 2026-04-27 ...       ← 最后读取时间（atime）
Modify: 2026-04-27 ...       ← 文件内容最后修改时间（mtime）
Change: 2026-04-27 ...       ← 文件元数据最后改变时间（ctime）
 Birth: -                    ← 创建时间（本文件系统不支持，故显示 -）
--------------------------------------------------
· 三个时间戳：
  atime (Access) – 读取文件内容时更新
  mtime (Modify) – 修改文件内容时更新
  ctime (Change) – 修改权限、所有者等元数据时更新（内容修改也会触发）
--------------------------------------------------
```
...
6. 字符设备文件：字符设备在应用层的访问接口。  鼠标  键盘  触摸屏    ---普通硬件设备
7. 块设备文件：块设备在应用层的访问接口。   U盘  固态硬盘  磁盘  --存储容量的硬件设备
...
**系统调用接口：内核与用户空间的一套通信接口**

**区别**：


- 系统IO：是众多系统调用当中专用于文件操作的一部分接口。存在于man手册中的第二章 man 2 open
- 标准IO：是众多标准函数当中专用于文件操作的一部分接口。存在于man手册中的第三章 man 3 fopen
...
>任何文件操作都必须先打开
>
>但 `stdin` 、 `stdout` 和 `stderr` 属于程序启动时自动为你打开的特殊文件
...
> fd
> ↓
> 打开文件对象 open file description （open 一次，内核都会创建一个新的打开文件对象）
> ↓
> 文件偏移量 offset (文件偏移量（offset）保存在内核打开文件对象中。)
> ```
>
> 
...
​	可选(0个/多个)  -> 如果选了，就使用"|"来添加

​	O_CREAT   -> 如果打开的那个文件不存在，那么就会创建。

​		  -> 如果flags中有O_CREAT，那么需要填写mode这个参数。

​		  -> 如果flags中没有O_CREAT，那么mode这个参数填了也没用。

​	O_TRUNC：如果文件存在并且是一个普通文件，而且打开方式是O_WRONLY/O_RDWR，那么这个文件就会被清空。

​	O_APPEND：以追加的方式打开文件，在每一次写之前，文件的光标定位在末尾。因为文件默认打开的时候，光标在开头
...
>
> 内核需要知道：现在有多少个fd指向这个ofd(打开文件对象).ref(引用计数），所以close即ofd.ref-1
>
> ref = 0，内核才会释放打开文件对象
...
函数 open() 的返回值，是一个整型 int 数据。这个整型数据，实际上是内核中的一个称为 fd_array（文件描述符表 File Descriptor Table） 的数组的下标：

> 即fd是文件描述符表中的索引或者说是数组下标
...
- 可以重复打开同一个文件，每次打开文件都会使内核产生系列结构体（ofd也不同），并得到不同的文件描述符
...
"标准输入"、"标准输出"、"标准出错"，他们其实是一个宏定义来的，是被定义在一个头文件中，头文件路径：/usr/include/unistd.h
...
`ulimit -n` 显示 `1048576`，是因为在现代 Linux 发行版中，`systemd` 接管了资源限制的初始化，它会根据物理内存动态调高文件描述符的硬限制。**1024 只是传统的软限制，并非系统能承受的最大值**。

它的底层逻辑分为以下几个层面：

- **内核天花板（`fs.nr_open`）**：这是 Linux 内核为单个进程分配文件描述符设定的绝对上限。从 `2.6.25` 内核开始，其默认值在源码中固定为 `1024*1024 = 1048576`。`ulimit -n` 的值永远无法超越此限制。
- **两级限制（软与硬）**：资源限制分两层。普通用户可自行将软限制提升至硬限制，而硬限制通常只能由 root 用户调高。历史上，许多发行版默认将软限制设为 `1024`，硬限制设为 `4096`。在主流使用 `/etc/security/limits.conf` 的旧版本系统中，若未显式配置，`ulimit -Hn` 常显示 `4096`。
- **`systemd` 的动态接管**：近年的系统通过 `systemd` 提升默认值。它会绕过传统的 `limits.conf`，在启动早期根据物理内存自动计算并提升硬限制，直接写入内核天花板 `fs.nr_open` 的值。因此，假如你的内存足够，`systemd` 直接帮你的系统把硬限制拉升到了 `1048576`，也就是内核默认的绝对上限。由于软限制缺省会继承硬限制，你直接运行 `ulimit -n` 看到的也就是 `1048576`。
...
> 文件读写就是完成数据在内存与磁盘之间的传输。读文件是指将数据从磁盘读取到内存中供程序使用，而写文件是将程序的输出结果从内存写入磁盘。
>
> ofd.ref不等于0
...
>  file offset
>
> 文件偏移量（offset）保存在内核打开文件对象（ofd）中。
...
![image-20260711141332754](./img/image-20260711141332754.png)
...
1、如果当前已经在开头了，还往前偏移会怎样？

lseek()函数会执行失败，返回-1，当前文件的定位还是在开头。
...
2、如果当前已经在文件的末尾了，还往后偏移会怎样？

会发生偏移，但是在windows看的时候，还是在文件的末尾。
...
int main(int argc,char **argv) { 
	int fd = open("./1.txt",O_RDWR); //文件内容：hello world 
     ...
     printf("ret:%d\n",ret); //偏移成功 ret == 21 
     //如果此时不写入数据，直接关闭文件，文件的大小还是原来的11字节 
     write(fd,"123",3); //此时文件大小为24个字节 
...
`dup()`会将指定的旧文件描述符 oldfd 复制一份，并返回一个系统当前未用的最小的新文件描述符。注意，此时这新旧两个文件描述符是可以互换的，因为它们本质上指涉的是同一个文件（ofd），因此它们共享文件的读写偏移量和文件的状态标签（同一个ofd，而ofd存储offset，flags，ref），比如使用lseek()对新文件描述符修改文件偏移量，这个操作会同时影响旧文件描述符oldfd，再如，使用read()对新文件描述符读取文件部分内容后，可以继续对旧文件描述符读取后续内容。
